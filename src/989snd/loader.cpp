...
u32 loader::read_bank(std::istream& in) {
  size_t origin = in.tellg();
  FileAttributes<3> attr;
  in.read((char*)(&attr), sizeof(attr));

  if (attr.type != 1 && attr.type != 3) {
    std::cerr << fmt::format("Error: File type {} not supported.", attr.type) << std::endl;
    return -1;
  }
  ...
  in.seekg(origin + attr.where[chunk::bank].offset, std::istream::beg);
  ...
  if (attr.num_chunks >= 2) {
    in.seekg(origin + attr.where[chunk::samples].offset, std::istream::beg);
  ...
  if (attr.num_chunks >= 3) {
    in.seekg(origin + attr.where[chunk::midi].offset, std::istream::beg);
    load_midi(in);
  }
...
void loader::load_midi(std::istream& in) {
  FileAttributes<1> attr;
  u32 cur = in.tellg();

  in.read((char*)&attr, sizeof(attr));
  in.seekg(cur + attr.where[0].offset, std::istream::beg);

  auto midi = std::make_unique<u8[]>(attr.where[0].size);
  in.read((char*)midi.get(), attr.where[0].size);

  auto h = (MIDIBlock*)midi.get();
  std::cerr << fmt::format("Loaded midi {:.4}", (char*)&h->ID) << std::endl;
